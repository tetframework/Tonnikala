...
class TestHtmlTemplates(unittest.TestCase):
    ...
    def assert_render_throws(self, exception_class, template, debug=False, **args):
        ...
        raise AssertionError(
            "rendering the template {} did not throw a {}".format(
                template, exception_class.__name__
            )
        )
    ...
    def test_if(self):
        ...
        self.are("<html></html>", fragment, flag=False)
        self.are(
            '<html><div id="a">was true</div></html>', fragment, flag=dict(something=1)
        )
        self.are("<html></html>", fragment, flag={})
        self.are("<html></html>", fragment, flag=None)
    ...
    def test_def(self):
        fragment = (
            '<html><py:def function="foo(bar)">bar: ${bar}</py:def>'
            '-${foo("baz")}-</html>'
        )

        self.are("<html>-bar: baz-</html>", fragment)
        self.assert_loader_throws(
            TemplateSyntaxError, '<html><py:def bar="foobar"></py:def></html>'
        )
    ...
    def disabled_test_strip_evalled_expression(self):
        ...
        fragment = '<html><div py:strip="foo()">bar</div></html>'
        self.are(
            "<html>bar</html>", fragment, foo=lambda x=iter([True, False]): next(x)
        )

        self.are(
            "<html><div>bar<div></html>",
            fragment,
    ...
    def test_comments(self):
        fragment = (
            "<html><!-- some comment here, passed verbatim " "<html></html> --></html>"
        )
        self.are(
            "<html><!-- some comment here, passed verbatim <html></html> " "--></html>",
            fragment,
    ...
    def test_empty_tags(self):
        ...
        self.are(
            "<html><script></script><script></script><br /><br /></html>", fragment
        )

    def test_closures(self):
        fragment = (
            '<html><a py:def="embed(func)">${func()}</a>'
    ...
    def test_literal_only_1_argument(self):
        self.assert_render_throws(
            TypeError, "<html>$literal(val, val)</html>", val="<br/>"
        )

    def test_invalid_expression_raises_template_syntax_error_and_multiline(self):
        self.assert_loader_throws(
            TemplateSyntaxError, '<html><py:for each="i in i ' 'i\n"></py:for></html>'
        )
    ...
    def test_dollars(self):
        ...
        self.are(
            "<html><script>$.fn $(abc) $a foobar</script></html>", fragment, a="foobar"
        )
    ...
    def test_translation(self):
        ...
        self.are(
            '<html alt="foo"> abc </html>', fragment, debug=False, translatable=True
        )
    ...
    def test_cdata_elements_not_translated(self):
        ...
        return self.are(
            "<html>b<script>a</script>b<style>a</style>b</html>",
            fragment,
            debug=False,
            translatable=True,
            gettext=gettext,
    ...
    def test_attrs(self):
        ...
        self.are(
            '<html><div foo="bar" baz="42"></div></html>',
            fragment,
            debug=False,
            foo=attrs,
        )
    ...
    def test_void_elements(self):
        self.are(
            '<html><img title="foo" /></html>',
            '<html><img title="foo"></html>',
            debug=False,
        )

    def assert_file_rendering_equals(
        self, input_file, output_file, debug=False, **context
    ):
    ...
    def test_invalid_control_attribute(self):
        ...
        self.assert_loader_throws(TemplateSyntaxError, "<html py:notcorrect></html>")

        self.assert_loader_throws(
            TemplateSyntaxError, "<html><py:notcorrect></py:notcorrect></html>"
        )
    ...
    def test_python_processing_instruction(self):
        ...
        self.are(
            "<html></html>", '<html><?python foo("baz")?></html>', debug=False, foo=bar
        )
