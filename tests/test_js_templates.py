...
def execute_nodejs_runner(template_name, context):
    ...
    process = subprocess.Popen(
        [node_exe, js_driver, template_name, context], stdout=subprocess.PIPE
    )
...
class TestJsTemplates(unittest.TestCase):
    ...
    def assert_compile_throws(self, exception_class, template):
        ...
        raise AssertionError(
            "Compiling template {} did not throw a {}".format(
    ...
    def test_for(self):
        ...
        self.are(
            "<html><div>0</div><div>1</div></html>", fragment, values=list(range(2))
        )

    def test_def(self):
        fragment = (
            '<html><js:def function="foo(bar)">bar: ${bar}</js:def>'
            '-${foo("baz")}-</html>'
        )
    ...
    def test_closures(self):
        fragment = (
            '<html><a js:def="embed(func)">${func()}</a>'
            '<js:for each="i in [0,1,2]"><js:def function="callable('
            ')">${i}</js:def>'
...
"""
    ...
    def test_extension(self):
        self.assert_file_rendering_equals('base.tk', 'base.tk', title='the
        base')
        self.assert_file_rendering_equals('child.tk', 'child.tk', title='the
        child')
